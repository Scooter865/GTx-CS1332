...
public class AVL<T extends Comparable<? super T>> {
    ...
    private AVLNode<T> rAdd(T data, AVLNode<T> node) {
        // Base case - hit the bottom of the tree, make new node
        if (node == null) {
            size++;
        ...
        // Do nothing if data is found in the tree
    ...
    private AVLNode<T> rotateLeft(AVLNode<T> currentNode) {
        AVLNode<T> promotedNode = currentNode.getRight();

        currentNode.setRight(promotedNode.getLeft());
        promotedNode.setLeft(currentNode);

        updateHeightAndBF(currentNode);
        updateHeightAndBF(promotedNode);
    ...
    private AVLNode<T> rotateRight(AVLNode<T> currentNode) {
        AVLNode<T> promotedNode = currentNode.getLeft();

        currentNode.setLeft(promotedNode.getRight());
        promotedNode.setRight(currentNode);
    ...
    private AVLNode<T> balance(AVLNode<T> currentNode) {
        ...
        if (currentNode.getBalanceFactor() < -1) { /* Condition for a right heavy tree. BF < -1 */
            if (currentNode.getRight().getBalanceFactor() == 1) { /* Condition for a right-left rotation. Right child BF = 1 */
                currentNode.setRight(rotateRight(currentNode.getRight()));
            }
            currentNode = rotateLeft(currentNode);
        } else if (currentNode.getBalanceFactor() > 1) { /* Condition for a left heavy tree. BF > 1*/
            if (currentNode.getLeft().getBalanceFactor() == -1) { /* Condition for a left-right rotation. Left child BF = -1 */
